...
public class AES
{
    ...
    public static int[] blockToCells(BigInteger block)
    {
        ...
        {
            String number = blockBits.substring(8 * i, 8 * (i + 1));
            cells[i] = Integer.parseInt(number, 2);
        }

        return cells;
    }

    public static BigInteger cellsToBlock(int[] cells)
    {

        String blockBits = "";
        for (int i = 0; i < 16; i++)
        {
            String number = Integer.toBinaryString(cells[i]);
    ...
    public static BigInteger subBytes(BigInteger ciphertext)
    {
        ...
        int[] cells = blockToCells(ciphertext);
    ...
    public static BigInteger mixColumns(BigInteger ciphertext)
    {
        ...
        return cellsToBlock(outputCells);
    }
    ...
    public static BigInteger encrypt(BigInteger pt, BigInteger key)
    {
        BigInteger[] roundKeys = keyExpansion(key);

        pt = addRoundKey(pt, roundKeys[0]);
        System.out.println("After intial round:");
        ...
        pt = subBytes(pt);
        pt = shiftRows(pt);
        pt = addRoundKey(pt, roundKeys[10]);
        System.out.println("After 10 round:");
        System.out.println("State Array: ");
        printStateArray(pt);
        System.out.println("\n");
    ...
    public static BigInteger decrypt(BigInteger ct, BigInteger key)
    {

        BigInteger[] roundKeys = keyExpansion(key);

        ct = addRoundKey(ct, roundKeys[10]);
        ct = shiftRowsDec(ct);
        ct = subBytesDec(ct);
        System.out.println("After 10 round:");
        System.out.println("State Array: ");
    ...
    public static void printStateArray(BigInteger in)
    {
        String s=in.toString(16);
        if(s.length()!=32)
        {
        ...
        for(int i=0; i<8; i += 2)
        {
            for (int j = 0; j < 4; j++)
            {
                System.out.print("" + s.charAt(j * 8 + i) + "" + s.charAt(j * 8 + i + 1) + "  ");
            }
            System.out.print("\n");
        }
    }
        ...
        switch (choice)
        {
            case 1:
                ...
                do {
                    if(flag!=0)
                    {
                        System.out.println("Please enter the Text in the specified format");
                    }
                    System.out.println("Enter Plain Text in Hexadecimal Format( 128 bits | 16 Bytes | 32 character for base 16 ):");
                    input = sc.nextLine();
                    flag++;
                }while(checkInput(input));

                BigInteger pt = new BigInteger(input, 16);
                ...
                do {
                    ...
                    System.out.println("Enter Key in Hexadecimal Format( 128 bits | 16 Bytes | 32 character for base 16 ):");
                    input = sc.nextLine();
                    flag++;
                }while(checkInput(input));

                BigInteger Ekey = new BigInteger(input, 16);

                BigInteger et=encrypt(pt,Ekey);
                System.out.println("Encrypted message : " + et.toString(16)+"\n");
                System.out.println("Decrypted message : " + decrypt(et, Ekey).toString(16));
                break;
            ...
            case 2:
                ...
                do {
                    ...
                    System.out.println("Enter Cypher Text in Hexadecimal Format( 128 bits | 16 Bytes | 32 character for base 16 ):");
                    input = sc.nextLine();
                    flag++;
                }while(checkInput(input));

                BigInteger ct = new BigInteger(input, 16);
                ...
                BigInteger Dkey = new BigInteger(input, 16);

                System.out.println("Decrypted message : " + decrypt(ct, Dkey).toString(16));
                break;
...
//Plain Text: 304005356a67dc0742f82c08a100492e
//Key :       004672422ae7897c5868392d826b5248
